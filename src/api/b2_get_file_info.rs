...
pub fn b2_get_file_info<T: AsRef<str>>(client: &Client, auth: &B2Auth, file_id: T) -> Result<B2FileInfo, Error> {
    let req_body = serde_json::to_string(&GetFileInfoBody {
        file_id: file_id.as_ref(),
    }).unwrap();

    let resp = match client.post(&auth.api_url_for("b2_get_file_info"))
